...
The parser returns either `{ candidateIdentifier, phase? }` or an unlinked result and never calls Linear.
...
2. `linear_session_attributions`: one row per `codex_sessions.session_id`, containing the title fingerprint, candidate identifier, optional phase, resolution status, optional Linear ID, attempt/success timestamps, and a sanitized failure category.
3. `linear_sync_runs`: durable startup/manual/event run status and outcome counts for the status API.

Resolution status uses the closed set `unlinked`, `unconfigured`, `pending`, `linked`, `not_found`, and `error`. A failed refresh of an existing linked issue retains `linked` and its Linear ID while recording the latest synchronization failure separately; transient infrastructure failure must not erase a valid link.
...
An `AttributionCoordinator` owns a coalescing work queue. After a source import commits, the ingestion coordinator passes the importer-returned touched session IDs to a callback; the attribution coordinator then loads their current titles and evaluates them. Network resolution occurs outside DuckDB transactions.

Correctness does not depend on that callback. At startup and during explicit synchronization, reconciliation compares every eligible session with its attribution row and title fingerprint. Missing or changed rows are reparsed. Not-found and retryable outcomes are eligible for a later manual or startup run. Cached linked issues older than the configured cache TTL are eligible for metadata refresh.

Candidates are grouped by normalized identifier, in-flight lookups are coalesced, and distinct lookups use bounded concurrency. One lookup result can update the issue cache and all still-current session candidates for that identifier. Before committing a result, the coordinator rechecks each session's title fingerprint so a slow response cannot reapply an obsolete title.
...
Attribution batches the issue-cache upsert, current-candidate recheck, session-attribution updates, and sync-run counters into short transactions after remote results are available.
...
- **[A title changes while a lookup is in flight]** → Store and recheck the title fingerprint before committing the result.
...
4. Add the parser, adapter, coordinator, and startup/shutdown wiring behind the optional configuration boundary.
5. Extend authored API models/controllers and regenerate tsoa routes, OpenAPI, and the frontend RTK Query client.
